    ...
    methods: 
    {
      ...
      downloadReport( report_id,report_tilte)
      {
        ...
        .then(response => 
        {
            ...
            const repor_link = document.createElement("a");
            repor_link.href = window.URL.createObjectURL(my_blob);
            repor_link.download = 'Reporte '+report_tilte+'_'+new Date().getTime()+'.xlsx'
            repor_link.click();
        })
        .catch(error => 
        {
